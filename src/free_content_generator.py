...
"""
    ...
    @staticmethod
    def _quality_feedback(script: Dict) -> str:
        """Return a safe retry instruction for older editorial callers."""
        missing = [
            label
            for key, label in (
                ("hook", "сильніший хук"),
                ("body", "конкретний розвиток"),
                ("payoff", "виразний фінал"),
                ("loop", "замкнений loop"),
            )
            if not str(script.get(key) or "").strip()
        ]
        if missing:
            return "Перепиши сценарій і додай: " + ", ".join(missing) + "."
        return (
            "Зроби сценарій конкретнішим, оригінальним і динамічним; "
            "посиль першу секунду, утримання та фінальний payoff без вигаданих фактів."
        )
